...
parser.add_argument('gpu', metavar='GPU', type=str,
                    help='GPU id to use (set -1 to force CPU).')

parser.add_argument('task', metavar='TASK', type=str,
                    help='task id to use (prefix for checkpoint files).')
def main():
    ...
    use_cuda = torch.cuda.is_available() and str(args.gpu) != '-1'
    if use_cuda:
    ...
    if args.pre:
        if os.path.isfile(args.pre):
            ...
            print("=> loaded checkpoint '{}' (epoch {})".format(args.pre, args.start_epoch))
        else:
...
def train(train_list, model, criterion, optimizer, epoch):
    ...
    for i, (img, target) in enumerate(train_loader):
        ...
        img = img.to(device)
        output = model(img)
        target = target.type(torch.float32).unsqueeze(0).to(device)
